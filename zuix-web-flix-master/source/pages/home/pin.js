/* global zuix */
'use strict';
zuix.controller(function(cp) {
  const zx = zuix; // shorthand
  let itemsList;

  cp.create = function() {
    let url = cp.view().attr('data-o-rssL');
   // let urlR = cp.view().attr('data-o-rss');
    // Use a proxy to prevent CORS policy restrictions errors
    fetchListL(url);
       //fetchListR(url);
  };

  function refresh() {
    const list = cp.field('listL');
   // const listL = cp.field('listL');
  //  const listR = cp.field('listR');
    if (itemsList != null) {
    /*  zx.$.each(itemsList, function(i, item) {
        const options = {
          lazyLoad: true,
          model: item
        };*/
         const options = {
          lazyLoad: true
         // model: item
        };
        let elx;
          // different layout for first 4 items (bigger)
          elx = zx.createComponent('pages/home/items_list/vid', options).container();
          
          // 2 columns layout
       //   el.setAttribute('class', 'gallery-item'); // <-- will this work?
        
  
        
      
        // center the list on wide screens
            //   listR.append(el);
           //    listL.append(el);
        list.append(elx);
   //  });
     zuix.componentize();
     }
     }

  // Download RSS feed
  function fetchListL(rssUrl) {
    zx.$.ajax({
      url: rssUrl,
      success: function(res) {
        itemsList = parseRss(res);
        refresh();
      },
      error: function(err) {
        // TODO: handle error
      }
    });
  }
function fetchListR(rssUrl) {
    zx.$.ajax({
      url: rssUrl,
      success: function(res) {
        itemsList = parseRss(res);
        refresh();
      },
      error: function(err) {
        // TODO: handle error
      }
    });
  }
  // Parse RSS feed and create a JSON object out of it
  function parseRss(rssText) {
    const items = [];
    const parser = new DOMParser();
    const doc = parser.parseFromString(rssText, "text/xml");
    let d = zx.$(doc);
    d.find('channel > item').each(function(i, el){
      const title = getText(this.find('title'));
      const description = getText(this.find('description'));
      const pubDate = getText(this.find('pubDate'));
      const link = getText(this.find('link'));
      const thumb = getText(this.find('thumb'));
      const id = getText(this.find('id'));
      const imageList = this.find('[medium="video"]');
      if (title !== '') {
        const images = [];
        // parse images to a json list
        imageList.each(function(i, el) {
          images.push({
            url: this.attr('url'),
            thumb: this.attr('thumb'),
            width:this.attr('width'),
            height: this.attr('height')
          });
        });
        let cover;
        let ihd;
        if (i < 6 && images[0] != null) 
     {   cover = images[0].url;
     thumb = images[0].thumb;}
        
//  ihd = images[1].width;
        else if (i > 3 && images[1] != null) cover = images[4].url;
        const date = pubDate; // TODO: format date
        items.push({
          title,
          id,
          link,
          cover,
          date,
          description,
          images,
          thumb
        });
      }
    });
    return items;
  }

  function getText(node) {
    let text;
    // if node is ZxQuery, then get underlying HTMLElement
    if (node.length() > 0) node = node.get();
    if (node != null && node.firstChild != null) {
      // get rid of CDATA wrapper eventually
      text = node.firstChild.wholeText;
    } else if (node != null) {
      // get value as is
      text = node.innerHTML;
    }
    return text;
  }
});
